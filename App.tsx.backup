...
import { NAV_LINKS, CARDS_DATA, LOGO_SVG, HERO_LOGO_SVG } from './constants';
...
// 1. Sidebar Component (Vertical Left)
const Sidebar = () => {
  return (
...
const NavItem = ({ icon, active }: { icon: string; active?: boolean }) => (
  <button 
...
const BookingCard: React.FC<{ data: BookingCardProps }> = ({ data }) => {
  return (
    <div className="group relative bg-[#101010] rounded-[5px] overflow-hidden border border-neon-cyan/50 hover:border-neon-cyan transition-all duration-300 flex flex-col h-full shadow-lg hover:shadow-[0_0_20px_rgba(34,211,238,0.1)] hover:-translate-y-2">
      ...
      {/* Content */}
      <div className="p-8 flex-grow flex flex-col items-start text-right relative z-10">
        <h3 className="text-white font-bold text-2xl md:text-3xl mb-4 group-hover:text-neon-cyan transition-colors">
            {data.title}
        </h3>
      ...
      <div className="mt-auto">
        <div className="bg-neon-cyan w-full px-6 py-5 flex justify-between items-center text-white">
            {/* Book Button - Right side (Start) in RTL */}
            <button className="flex items-center gap-2 font-bold hover:opacity-75 transition-opacity text-lg">
             ...
             <div className="flex flex-col items-end leading-tight text-right">
                ...
                <span className="text-2xl font-extrabold flex items-center gap-1">
                    {data.price} <span className="text-base font-bold">ر.س</span>
                </span>
            </div>
        </div>
      </div>
...
const AnimatedCard: React.FC<{ data: BookingCardProps; index: number }> = ({ data, index }) => {
...
// 5. Filter/Tabs Section
const FilterBar = () => {
    return (
        <div className="w-full flex justify-between items-end px-4 md:px-12 py-6 border-b border-gray-800/50 mb-8">
...
// 7. Room Carousel Section Component
const RoomCarouselSection = () => {
  const { ref, isVisible } = useScrollAnimation();
  const [activeIndex, setActiveIndex] = useState(1); // Start with middle card
  
  // Placeholder room data - replace with actual images
  const rooms = [
    { id: 1, name: "غرفة VR", image: "./Rectangle 23.png" },
    { id: 2, name: "غرفة السباق", image: "./Rectangle 23.png" },
    { id: 3, name: "غرفة السوني", image: "./Rectangle 23.png" },
  ];
